...
import { traderAddressAtom } from '../config';

export const TxnsTab: React.FC = memo(() => {
  const tokenId = useTokenAddress();
  const { data: symbol } = useTokenInfo((data) => data?.baseAsset.symbol);
  const [traderAddressFilter, setTraderAddressFilter] = useAtom(traderAddressAtom);

  const { data, isFetching, fetchNextPage, hasNextPage } = useInfiniteQuery({
    ...ApeQueries.tokenTxs({ id: tokenId || '', traderAddress: traderAddressFilter }),
    enabled: !!tokenId,
  });
  ...
  return (
    <TxTable
      ...
      paused={paused}
      setPaused={setPaused}
      traderAddressFilter={traderAddressFilter}
      setTraderAddressFilter={setTraderAddressFilter}
    />
  );
});
