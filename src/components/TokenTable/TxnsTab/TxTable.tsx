...
type TxTableProps<TData, TValue> = {
  ...
  traderAddressFilter?: string | undefined;
  setTraderAddressFilter: (address: string | undefined) => void;
};
...
export function TxTable<TData, TValue>({
  ...
  const table = useReactTable({
    ...
    // Server-side filtering is used via API, no client-side filtering needed
    meta: {
      dateMode,
      setDateMode,
      walletAddress,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {traderAddressFilter && (
        <div className="flex items-center justify-between gap-2 px-3 py-2 bg-neutral-800/50 border-b border-neutral-800 text-xs shrink-0">
          <div className="flex items-center gap-2 text-neutral-300">
            <span className="iconify ph--funnel-bold text-primary" />
            <span>Filtering by trader:</span>
            <TruncatedAddress
              className="font-medium text-white"
              address={traderAddressFilter}
              charsStart={4}
              charsEnd={4}
            />
          </div>
          <button
            type="button"
            onClick={() => setTraderAddressFilter(undefined)}
            className="flex items-center gap-1 px-2 py-1 rounded text-neutral-400 hover:text-white hover:bg-neutral-700 transition-colors"
          >
            <span className="iconify ph--x-bold" />
            <span>Clear</span>
          </button>
        </div>
      )}
      ...
      <div ref={parentRef} className="flex-1 overflow-y-auto">
        ...
        >
          <Table className="text-xs">
            <TableHeader className="sticky -top-px z-10 bg-neutral-900">
