  ...
  useDataStreamListener(
    ...
    useCallback((_get, _set, msg) => {
      ...
      if (!resolution || !baseAsset || !resolutionToMostRecentBar) {
        // Expected during initial page load - data not yet available
        return;
      }
      ...
      if (filteredTxs.length === 0) {
        // No relevant transactions in this batch
        return;
      }
      ...
      if (!recentBar) {
        // Chart not yet initialized with historical bars
        return;
      }
      ...
      if (!nextBar) {
        // Unable to compute next bar from transactions
        return;
      }
      ...
      if (!callback) {
        // Chart callback not yet registered
        return;
      }
      callback(nextBar);
    }, [])
